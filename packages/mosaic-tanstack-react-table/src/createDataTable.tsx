...
// React Factory for creating DataTable components.
// Instrumenting with lifecycle logging to debug mount/unmount and error boundaries.
import React, {
  Fragment,
  useEffect,
  useRef,
...
function useColumnResizing(table: TanstackTable<any>) {
  ...
  useEffect(() => {
    const handleMove = (event: PointerEvent) => {
      ...
      setResizeState((prev) => ({
        ...prev,
        indicatorOffset: newSize - dragInfo.current.startSize,
      }));
    };
...
function TableUI<TData extends object>({
  table,
  containerRef,
  getResizeHandler,
  resizeState,
                ...
                return (
                  ...
                  >
                    ...
                    {header.column.getCanResize() && (
                      <>
                        <div
                          onPointerDown={getResizeHandler(
                            header.column.id,
                            header.getSize(),
                          )}
                        ...
                        {resizeState.isResizing &&
                          resizeState.resizingColumnId === header.column.id && (
                            <div
                              className="resize-indicator-guide"
                              style={{
                                transform: `translateX(${resizeState.indicatorOffset}px)`,
                              }}
                            />
                          )}
                      </>
          ...
          {virtualRows.map((virtualRow) => {
            ...
            return (
              <tr
                key={row.id}
                onMouseEnter={() =>
                  table.options.meta?.onRowHover?.(row.original)
                }
                onClick={() => table.options.meta?.onRowClick?.(row.original)}
                style={{
                  height: `${virtualRow.size}px`,
                  cursor:
                    table.options.meta?.onRowHover ||
              ...
              >
                {row.getVisibleCells().map((cell) => (
                  <td
                    key={cell.id}
                    style={{ padding: '4px', borderTop: '1px solid #eee' }}
                  >
  ...
  return function CreatedTableComponent(props: CreatedTableProps) {
    ...
    return (
      <Fragment>
        ...
        {isLookupPending && (
          <div style={{ color: 'blue' }}>Applying advanced filter...</div>
        )}
