 ...
 * dashboard state.
 */
export function MosaicProvider({ selections: selectionConfigs, children }: MosaicProviderProps) {
...
export function useMosaicSelection(name: string): Selection {
  ...
  if (!selection) {
      throw new Error(`Mosaic Selection "${name}" is not registered in the MosaicProvider.`);
  }
  return selection;
}