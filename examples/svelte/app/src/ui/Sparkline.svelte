<script lang="ts">
  ...
  $: isDataPlottable =
    plainData.length > 0 &&
    !plainData.some((d) => typeof d !== 'number' || !isFinite(d));
...
>
  {#if !isDataPlottable}
    <div
      style="height: {height +
        20}px; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #999;"
    >
      (no data)
    </div>
  {:else if plainData.length === 1}
    ...
    >
      ...
      {#if startDate}
        <div
          style="text-align: center; font-size: 10px; color: #666; margin-top: 2px; opacity: {hoverPoint
            ? 1
            : 0.5}"
        >
  ...
  {:else}
    ...
    {#if startDate && endDate}
      <div
        style="display: flex; justify-content: space-between; font-size: 10px; color: #666; margin-top: 4px; opacity: {hoverPoint
          ? 1
          : 0.5}; transition: opacity 0.2s ease-in-out;"
      >
        <span>{formatDate(startDate)}</span>
