...
export const Sparkline: React.FC<SparklineProps> = ({
  ...
  const isDataPlottable = useMemo(() => {
    ...
    const containsInvalidNumbers = plainData.some(d => typeof d !== 'number' || !isFinite(d));
  ...
  const { path, xScale, yScale } = useMemo(() => {
    if (!isDataPlottable || plainData.length < 2) return { path: null, xScale: null, yScale: null };
  ...
  if (!isDataPlottable) {
    return <div style={{height: `${height + 20}px`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: '#999'}}>(no data)</div>;
  }
  ...
  if (plainData.length === 1) {
    ...
    return (
        <div 
            onMouseEnter={() => setHoverPoint({ index: 0, value, x: width / 2, y: height / 2 })}
        ...
        >
            <svg width={width} height={height} style={{ overflow: 'visible' }}>
                ...
                {hoverPoint && (
                    <g>
                        <text x={width / 2} y={height / 2} dy={-10} textAnchor="middle" fontSize="12" fill="#333" fontWeight="bold">
                            ${value.toFixed(0)}
                        </text>
                    </g>
                )}
            ...
            {startDate && (
                 <div style={{ textAlign: 'center', fontSize: '10px', color: '#666', marginTop: '2px', opacity: hoverPoint ? 1 : 0.5 }}>
  ...
  return (
    <div style={{ position: 'relative', cursor: 'crosshair', height: `${height + 20}px` }}>
        <svg width={width} height={height} style={{ overflow: 'visible' }}>
            <path d={path || ''} fill="none" stroke={color} strokeWidth={strokeWidth} />
            
            {hoverPoint && (
                <g style={{ pointerEvents: 'none' }}>
                    <line x1={hoverPoint.x} y1={0} x2={hoverPoint.x} y2={height} stroke="#aaa" strokeWidth="1" strokeDasharray="2,2" />
                    <circle cx={hoverPoint.x} cy={hoverPoint.y} r={strokeWidth + 1.5} fill="white" stroke={color} strokeWidth="1.5" />
                    <text x={hoverPoint.x} y={hoverPoint.y} dy={-10} textAnchor="middle" fontSize="12" fill="#333" fontWeight="bold">
        ...
        {startDate && endDate && (
             <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#666', marginTop: '4px', opacity: hoverPoint ? 1 : 0.5, transition: 'opacity 0.2s ease-in-out' }}>
                <span>{formatDate(startDate)}</span>
