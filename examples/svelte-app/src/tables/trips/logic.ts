// src/lib/tables/trips/logic.ts
// This file contains the framework-agnostic data and logic configuration for the Trips table.
import type { DataTableLogicConfig, TripSummary } from '@mosaic-tanstack/core';
...
export const tripsLogicConfig: DataTableLogicConfig<TripSummary> = {
    ...
    columns: [
      { id: 'dropoff_zone', accessorKey: 'dropoff_zone', enableSorting: false, enableColumnFilter: false },
    ...
    getBaseQuery: (filters) => {
        ...
        return Query.from(aggregation)
            .select('*', { dropoff_zone: vg.sql`'(' || zone_x || ', ' || zone_y || ')'` });
    },
    hoverInteraction: {
        createPredicate: (row) => {
