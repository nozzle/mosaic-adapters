...
export function Sparkline(props: {
  ...
  // A closed path from the line down to the baseline and back, for the area fill.
  const areaPath = useMemo(() => {
    ...
    return `${path} L${last.x.toFixed(1)},${INNER_H} L${first.x.toFixed(1)},${INNER_H} Z`;
  }, [path, scaled]);
  ...
  // Fewer than two points is not a trend: render a muted en-dash rather than a
  // lone dot, so an empty or single-sample series reads as "no data".
  if (scaled.length < 2) {
    return (
      <div
  ...
  const endpoint = scaled[scaled.length - 1];

  return (
    <div
      data-testid="sparkline"
      className="relative inline-block text-gf-green"
      style={{ width: WIDTH, height: HEIGHT }}
    >
      <svg
        width={WIDTH}
      ...
      {hovered !== null ? (
        <div
          className="pointer-events-none absolute z-50 rounded-gf border border-line bg-editor px-1.5 py-1 text-[10px] leading-tight whitespace-nowrap text-editor-ink shadow-lg"
