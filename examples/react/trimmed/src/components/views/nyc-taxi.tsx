...
// Charts need to react to Map Brush, Detail Filters, AND the calculated Zone Filter.
...
const $chartContext = vg.Selection.intersect({
  include: [$brush, $detailFilter, $zoneFilter],
});
...
export function NycTaxiView() {
  ...
  // --- Zone Filter Bridge ---
  // This effect listens to the Summary Table Filters (Count > X) and the Context (Map Brush).
  ...
  useEffect(() => {
    const updateZoneFilter = () => {
      ...
      // Get the context predicate (Map Brush + Time Filters)
    ...
    // If the Context changes (Map Brush), the valid zones might change (counts shift).
