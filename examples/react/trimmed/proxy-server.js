...
app.post('/query', async (req, res) => {
  ...
  if (!req.body || typeof req.body.sql !== 'string') {
    console.log(
      '[Proxy] Received potentially malformed body:',
      JSON.stringify(req.body),
    );
  }

  console.log(
    `[Proxy #${reqId}] Forwarding: "${querySummary}" to ${REMOTE_DB_URL}`,
  );
  console.log(
  ...
  try {
    ...
    console.log(
      `[Proxy #${reqId}] Upstream Response: ${backendResponse.status} (${Date.now() - start}ms)`,
    );
