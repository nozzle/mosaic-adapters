...
app.post('/query', async (req, res) => {
  ...
  if (req.body && req.body.sql) {
    ...
    if (typeof sqlParam === 'string') {
      querySummary =
        sqlParam.substring(0, 50) + (sqlParam.length > 50 ? '...' : '');
    } else {
      // If it's not a string (e.g. object/array), stringify it for logs
      try {
        querySummary =
          JSON.stringify(sqlParam).substring(0, 50) +
          '... (Type: ' +
          typeof sqlParam +
          ')';
      } catch (e) {
  ...
  if (!req.body || typeof req.body.sql !== 'string') {
    console.log(
      '[Proxy] ⚠️ Received potentially malformed body:',
      JSON.stringify(req.body),
    );
  }

  console.log(
    `[Proxy #${reqId}] ➡️ Forwarding: "${querySummary}" to ${REMOTE_DB_URL}`,
  );
  console.log(
    `[Proxy #${reqId}] 🔒 Auth Headers: X-Tenant-Id=${TENANT_ID ? 'Set' : 'MISSING'}`,
  );
  ...
  try {
    ...
    console.log(
      `[Proxy #${reqId}] ⬅️ Upstream Response: ${backendResponse.status} (${Date.now() - start}ms)`,
    );
    ...
    console.log(
      `[Proxy #${reqId}] ✅ Sending ${buffer.byteLength} bytes to client (${contentType})`,
    );
    ...
    const isDescQuery =
      originalSql &&
      typeof originalSql === 'string' &&
      originalSql.trim().toUpperCase().startsWith('DESC');
    if (contentType.includes('arrow')) {
      const queryPrefix = originalSql
        ? originalSql.substring(0, 60)
        : 'unknown';
      console.log(
        `[Proxy #${reqId}] 🔍 Arrow (${buffer.byteLength} bytes): ${queryPrefix}...`,
      );
      try {
        const table = tableFromIPC(new Uint8Array(buffer));
        console.log(
          `[Proxy #${reqId}]   numRows: ${table.numRows}, cols: [${table.schema.fields.map((f) => f.name).join(', ')}]`,
        );
        if (isDescQuery) {
          if (table.numRows > 0) {
            const firstRow = table.get(0);
            console.log(
              `[Proxy #${reqId}]   DESC row:`,
              JSON.stringify(firstRow),
            );
          } else {
            console.error(
              `[Proxy #${reqId}]   ❌ DESC EMPTY! Will cause 'column_type' undefined error!`,
            );
          }
        }
      } catch (arrowErr) {
