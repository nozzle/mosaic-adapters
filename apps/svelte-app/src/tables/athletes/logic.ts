...
import type { DataTableLogicConfig, Athlete } from '@mosaic-tanstack/core';
...
export const athletesLogicConfig: DataTableLogicConfig<Athlete> = {
    ...
    columns: [
        ...
        { id: 'rank', accessorKey: 'rank', enableSorting: true, enableColumnFilter: false },
        ...["name", "nationality", "sex", "height", "weight", "sport"].map(id => ({
            id: id,
    ...
    getBaseQuery: (filters) => {
        ...
        return Query.from('athletes').where(where).select('*', { rank: rankExpression });
    },
    hoverInteraction: {
